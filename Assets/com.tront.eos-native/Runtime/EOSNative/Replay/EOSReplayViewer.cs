...
namespace EOSNative.Replay
{
    ...
    public class EOSReplayViewer : MonoBehaviour
    {
        ...
        /// Override this via OnTargetCycled event for custom spectator integration.
        /// </summary>
        public string GetCurrentTargetName()
        {
            var targets = GetReplayTargets();
            if (targets.Count == 0) return "No Target";
            int idx = Mathf.Clamp(_currentTargetIndex, 0, targets.Count - 1);
            return targets[idx]?.name ?? "No Target";
        }

        /// <summary>
        /// Cycle to next/previous spectator target among replay player objects.
        /// </summary>
        public void CycleTarget(int direction)
        {
            var targets = GetReplayTargets();
            if (targets.Count == 0) return;
            _currentTargetIndex = (_currentTargetIndex + direction + targets.Count) % targets.Count;
        }
