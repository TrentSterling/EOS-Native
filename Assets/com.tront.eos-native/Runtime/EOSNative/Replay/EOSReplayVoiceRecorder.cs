...
namespace EOSNative.Replay
{
    ...
    /// Call StartRecording/StopRecording manually or integrate with your own replay system.
    /// </summary>
    public class EOSReplayVoiceRecorder : MonoBehaviour
    {
        ...
        public ReplayVoiceTrack StopRecording()
        {
            ...
            EOSDebugLogger.Log(DebugCategory.Replay, "EOSReplayVoiceRecorder",
                $"Stopped voice recording: {track.Segments.Count} segments");
            ...
            lock (_bufferLock)
            {
                if (!_voiceBuffers.TryGetValue(puid, out var buffer))
                {
                    ...
                    if (!CurrentTrack.Speakers.ContainsKey(puid))
                    {
                        ...
                        string name = registry?.GetPlayerName(puid) ?? puid.Substring(0, 8);
