    ...
    public class EOSAntiCheatManager : MonoBehaviour
    {
        ...
        public Result BeginSession()
        {
            ...
            else
            {
                ...
                EOSDebugLogger.LogError("EOSAntiCheatManager", $"Failed to begin session: {result}");
            }
        ...
        public Result EndSession()
        {
            ...
            {
                EOSDebugLogger.LogError("EOSAntiCheatManager", $"Failed to end session: {result}");
            }
        ...
        public IntPtr RegisterPeer(string puid, string ipAddress = null)
        {
            ...
            {
                EOSDebugLogger.LogError("EOSAntiCheatManager", $"Failed to register peer {puid}: {result}");
                return IntPtr.Zero;
            }
        }
        ...
        private void OnClientIntegrityViolated(ref OnClientIntegrityViolatedCallbackInfo info)
        {
            ...
            EOSDebugLogger.LogError("EOSAntiCheatManager",
        ...
        private void OnMessageToPeer(ref Epic.OnlineServices.AntiCheatCommon.OnMessageToClientCallbackInfo info)
        {
            if (info.MessageData.Array != null)
            {
                var data = new byte[info.MessageData.Count];
                Array.Copy(info.MessageData.Array, info.MessageData.Offset, data, 0, info.MessageData.Count);
                _outgoingPeerMessages.Enqueue((info.ClientHandle, data));
            }
        }

        private void OnPeerActionRequiredCallback(ref Epic.OnlineServices.AntiCheatCommon.OnClientActionRequiredCallbackInfo info)
        {
            string puid = GetPeerPuid(info.ClientHandle) ?? "Unknown";
            ...
            OnPeerActionRequired?.Invoke(info.ClientHandle, info.ClientAction, reasonStr);

            // Auto-handle common actions
            if (info.ClientAction == AntiCheatCommonClientAction.RemovePlayer)
            {
                UnregisterPeer(info.ClientHandle);
            }
        }

        private void OnPeerAuthStatusChangedCallback(ref Epic.OnlineServices.AntiCheatCommon.OnClientAuthStatusChangedCallbackInfo info)
        {
            string puid = GetPeerPuid(info.ClientHandle) ?? "Unknown";
            EOSDebugLogger.Log(DebugCategory.Sanctions, "EOSAntiCheatManager",
                $"Peer auth status changed: {puid} - {info.ClientAuthStatus}");

            OnPeerAuthStatusChanged?.Invoke(info.ClientHandle, info.ClientAuthStatus);
        }

        private void OnLobbyJoined(LobbyData lobby)
        {
            if (_autoStartSession && !IsSessionActive)
            {
                BeginSession();
            }
        }
