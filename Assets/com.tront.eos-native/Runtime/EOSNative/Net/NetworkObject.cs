...
namespace EOSNative.Net
{
    ...
    public class NetworkObject : MonoBehaviour
    {
        ...
        public SyncVar<T> Sync<T>(T defaultValue = default)
        {
            ...
            var syncVar = new SyncVar<T>(this, defaultValue, index);
        ...
        /// </summary>
        public SyncList<T> SyncList<T>(List<T> initial = null)
        {
            if (_syncVars.Count >= 32)
                throw new InvalidOperationException(
            ...
            byte index = (byte)_syncVars.Count;
            var syncList = new SyncList<T>(this, initial ?? new List<T>(), index);
        ...
        /// </summary>
        public SyncDictionary<TKey, TValue> SyncDictionary<TKey, TValue>(Dictionary<TKey, TValue> initial = null)
        {
            if (_syncVars.Count >= 32)
                throw new InvalidOperationException(
            ...
            byte index = (byte)_syncVars.Count;
            var syncDict = new SyncDictionary<TKey, TValue>(this, initial ?? new Dictionary<TKey, TValue>(), index);
