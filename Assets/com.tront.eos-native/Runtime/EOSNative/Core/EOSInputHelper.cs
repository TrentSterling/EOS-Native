...
namespace EOSNative
{
    ...
    /// Requires the Unity Input System package for automatic detection.
    /// Without it, utility methods (parsing, display names, etc.) still work.
    /// </summary>
    public static class EOSInputHelper
    {
...
#else
            // Without Input System, detect based on platform
            DetectPlatformInputType();
#endif
        }
        ...
        private static void OnDeviceChange(InputDevice device, InputDeviceChange change)
        {
            ...
            {
                // A new device was connected - wait for actual input
            }
...
#else
        private static void DetectPlatformInputType()
        {
            if (EOSPlatformHelper.IsVR)
            {
                SetInputType(InputType.VRController);
            }
            else if (EOSPlatformHelper.IsMobile)
            {
                SetInputType(InputType.Touch);
            }
            else
