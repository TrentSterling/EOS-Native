...
namespace EOSNative.Social
{
    ...
    public class EOSGlobalChatManager : MonoBehaviour
    {
        ...
        private IEnumerator InitializeCoroutine()
        {
            ...
            _localDisplayName = EOSPlayerRegistry.Instance?.GetOrGenerateName(_localPuid) ?? "Player";
        ...
        /// </summary>
        public Task<Result> JoinChannelAsync(string channelName)
        {
            if (string.IsNullOrEmpty(channelName))
                return Task.FromResult(Result.InvalidParameters);

            channelName = NormalizeChannelName(channelName);

            if (_subscribedChannels.ContainsKey(channelName))
            {
                return Task.FromResult(Result.AlreadyPending);
            }

            if (_subscribedChannels.Count >= _maxChannels)
            {
                EOSDebugLogger.LogWarning(DebugCategory.Social, "EOSGlobalChatManager",
                    $"Cannot join more than {_maxChannels} channels");
                return Task.FromResult(Result.LimitExceeded);
            }
        ...
        /// </summary>
        public Task<Result> LeaveChannelAsync(string channelName)
        {
            channelName = NormalizeChannelName(channelName);
        ...
        /// </summary>
        public Task<Result> SendMessageAsync(string channelName, string message)
        {
            channelName = NormalizeChannelName(channelName);
