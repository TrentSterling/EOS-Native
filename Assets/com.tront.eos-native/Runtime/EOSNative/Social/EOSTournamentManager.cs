...
#pragma warning disable CS0414 // Setting for future bracket generation with byes
...
#pragma warning restore CS0414
        ...
        public Tournament CreateTournament(string name, TournamentFormat format = TournamentFormat.SingleElimination)
        {
            ...
            {
                EOSDebugLogger.LogWarning(DebugCategory.Social, "EOSTournamentManager", "Cannot create tournament while one is active");
                return null;
            }
        ...
        public bool RegisterParticipant(string participantId, string name, int? seed = null)
        {
            ...
            {
                EOSDebugLogger.LogWarning(DebugCategory.Social, "EOSTournamentManager", "No active tournament");
                return false;
            }
            ...
            if (ActiveTournament.State != TournamentState.Registration)
            {
                ...
                {
                    EOSDebugLogger.LogWarning(DebugCategory.Social, "EOSTournamentManager", "Registration is closed");
                    return false;
                }
            }
            ...
            {
                EOSDebugLogger.LogWarning(DebugCategory.Social, "EOSTournamentManager", "Tournament is full");
                return false;
            }

            if (ActiveTournament.Participants.Any(p => p.Id == participantId))
            {
                EOSDebugLogger.LogWarning(DebugCategory.Social, "EOSTournamentManager", "Participant already registered");
                return false;
            }
        ...
        public bool UnregisterParticipant(string participantId)
        {
            ...
            {
                EOSDebugLogger.LogWarning(DebugCategory.Social, "EOSTournamentManager", "Cannot unregister during active tournament");
                return false;
            }
        ...
        public bool StartTournament()
        {
            ...
            if (ActiveTournament.State != TournamentState.Registration)
            {
                EOSDebugLogger.LogWarning(DebugCategory.Social, "EOSTournamentManager", "Tournament already started");
                return false;
            }

            if (ActiveTournament.Participants.Count < 2)
            {
                EOSDebugLogger.LogWarning(DebugCategory.Social, "EOSTournamentManager", "Need at least 2 participants");
                return false;
            }
        ...
        public bool ReportMatchResult(string matchId, string winnerId, int winnerScore = 0, int loserScore = 0)
        {
            ...
            if (match == null)
            {
                EOSDebugLogger.LogWarning(DebugCategory.Social, "EOSTournamentManager", $"Match not found: {matchId}");
                return false;
            }

            if (match.State != MatchState.Ready && match.State != MatchState.InProgress)
            {
                EOSDebugLogger.LogWarning(DebugCategory.Social, "EOSTournamentManager", "Match is not ready to report");
                return false;
            }
            ...
            {
                EOSDebugLogger.LogWarning(DebugCategory.Social, "EOSTournamentManager", "Winner not in match");
                return false;
            }
